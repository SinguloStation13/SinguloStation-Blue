	...
	if(lit && cig && user.a_intent == INTENT_HELP)
		...
		if(M == user)
			...
			if(cig.reagents.get_reagent_amount(/datum/reagent/toxin/plasma))
				...
				log_game("[cig] that contains plasma was lit by [ADMIN_LOOKUPFLW(user)] for [key_name_admin(M)]!")
			if(cig.reagents.get_reagent_amount(/datum/reagent/fuel))
				...
				log_game("[cig] that contains fuel was lit by [ADMIN_LOOKUPFLW(user)] for [key_name_admin(M)]!")
	...
	if(!lit && smoketime > 0)
		...
		if(lighting_text)
			if(src.reagents.get_reagent_amount(/datum/reagent/toxin/plasma))
				...
				log_game("[src] that contains plasma was lit by [ADMIN_LOOKUPFLW(user)]!")
			if(src.reagents.get_reagent_amount(/datum/reagent/fuel))
				...
				log_game("[src] that contains fuel was lit by [ADMIN_LOOKUPFLW(user)]!")
			light(lighting_text)
	else
		return ..()
...
/obj/item/clothing/mask/cigarette/afterattack(obj/item/reagent_containers/glass/glass, mob/user, proximity)
	...
	if(istype(glass))	//you can dip cigarettes into beakers
		if(glass.reagents.trans_to(src, chem_volume, transfered_by = user))	//if reagents were transfered, show the message
			...
			log_game("[user] added reagents to [src], it now contains [english_list(src.reagents.reagent_list)]!")
	...
	if(lit && cig && user.a_intent == INTENT_HELP)
		...
		if(M == user)
			if(cig.reagents.get_reagent_amount(/datum/reagent/toxin/plasma))
				...
				log_game("[cig] that contains plasma was lit by [ADMIN_LOOKUPFLW(user)]!")
			if(cig.reagents.get_reagent_amount(/datum/reagent/fuel))
				message_admins("[cig] that contains fuel was lit by [ADMIN_LOOKUPFLW(user)]!")
				log_game("[cig] that contains fuel was lit by [ADMIN_LOOKUPFLW(user)]!")
			cig.attackby(src, user)
		else
			if(cig.reagents.get_reagent_amount(/datum/reagent/toxin/plasma))
				message_admins("[cig] that contains plasma was lit by [ADMIN_LOOKUPFLW(user)] for [key_name_admin(M)]!")
		...
		if(M == user)
			...
			if(fancy)
				if(cig.reagents.get_reagent_amount(/datum/reagent/toxin/plasma))
					...
					log_game("[cig.name] that contains plasma was lit by [ADMIN_LOOKUPFLW(user)] for [key_name_admin(M)]!")
				if(cig.reagents.get_reagent_amount(/datum/reagent/fuel))
					message_admins("[cig.name] that contains fuel was lit by [ADMIN_LOOKUPFLW(user)] for [key_name_admin(M)]!")
					log_game("[cig.name] that contains fuel was lit by [ADMIN_LOOKUPFLW(user)] for [key_name_admin(M)]!")
